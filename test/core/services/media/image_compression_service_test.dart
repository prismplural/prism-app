...
void main() {
  ...
  group('ImageCompressionService.computeBlurhashFromImage', () {
    ...
    test(
      'bounds the blurhash resize for an extreme-tall image (no OOM)',
      () async {
  ...
  group('ImageCompressionService.compressImage', () {
    ...
    test(
      'uses encoded bytes for metadata when Dart cannot decode source',
      () async {
        final normalized = img.Image(width: 32, height: 18);
        img.fill(normalized, color: img.ColorRgb8(120, 60, 200));
        final normalizedBytes = Uint8List.fromList(img.encodeJpg(normalized));
        ...
        final compressed = await ImageCompressionService(
          encodeImage:
              ({
                ...
                expect(imageBytes, source);
                expect((maxWidth, maxHeight), (2048, 2048));
                expect(quality, 85);
                return (normalizedBytes, 'image/jpeg');
              },
        ).compressImage(source);

        expect(compressed.mimeType, 'image/jpeg');
        expect((compressed.width, compressed.height), (32, 18));
        expect(compressed.bytes, normalizedBytes);
        expect(compressed.blurhash, isNotEmpty);
      },
    );

    test(
      'generates thumbnails through the encoder when Dart cannot decode source',
      () async {
        final thumbnail = img.Image(width: 30, height: 20);
        img.fill(thumbnail, color: img.ColorRgb8(20, 120, 60));
        ...
        final generated = await ImageCompressionService(
          encodeImage:
              ({
                required imageBytes,
                required maxWidth,
                ...
                expect(imageBytes, source);
                expect((maxWidth, maxHeight), (300, 300));
                expect(quality, 85);
                return (thumbnailBytes, 'image/jpeg');
              },
        ).generateThumbnail(source);

        expect(generated, thumbnailBytes);
      },
    );
