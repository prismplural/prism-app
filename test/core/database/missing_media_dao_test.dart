...
void main() {
  ...
  test('markMissing creates a pending, immediately-eligible entry', () async {
    ...
    expect((await dao.dueForRequest(5000)).single.forceRepair, isFalse);
  });

  test(
    'markMissing is idempotent: preserves the terminal clock + cooldown',
    () async {
      await dao.markMissing(
        mediaId: 'a',
        priority: MissingMediaDao.priorityChat,
  ...
  test(
    'markMissing upgrades a pending entry to repair without resetting cooldown',
    () async {
      await dao.markMissing(
        mediaId: 'a',
        priority: MissingMediaDao.priorityChat,
      ...
      final row = await dao.getById('a');
      expect(row!.state, MissingMediaDao.statePendingRepair);
      expect(row.attempts, 2);
      expect(row.nextEligibleAt, 9000);
      expect((await dao.dueForRequest(9000)).single.forceRepair, isTrue);
    },
  );
  ...
  test(
    'markTerminal removes the entry from pending/due but retains it',
    () async {
      await dao.markMissing(
        mediaId: 'a',
        priority: MissingMediaDao.priorityChat,
  ...
  test('markTerminal preserves repair intent', () async {
    await dao.markMissing(
      mediaId: 'a',
      priority: MissingMediaDao.priorityChat,
      nowMs: 1000,
  ...
  test(
    'requestAllNow revives terminal + pending entries for an immediate retry',
    () async {
      await dao.markMissing(
        mediaId: 'a',
        priority: MissingMediaDao.priorityChat,
  ...
  test('requestAllNow preserves repair intent for revived entries', () async {
    await dao.markMissing(
      mediaId: 'a',
      priority: MissingMediaDao.priorityChat,
      nowMs: 1000,
    ...
    final row = await dao.getById('a');
    expect(row!.state, MissingMediaDao.statePendingRepair);
    expect((await dao.dueForRequest(2000)).single.forceRepair, isTrue);
  });

  test('requestAllNow can promote legacy entries to repair', () async {
    await dao.markMissing(
      mediaId: 'pending',
      priority: MissingMediaDao.priorityChat,
      nowMs: 1000,
    ...
    await dao.markTerminal('terminal');

    await dao.requestAllNow(2000, promoteToRepair: true);

    expect(
      (await dao.getById('pending'))!.state,
      MissingMediaDao.statePendingRepair,
    );
    expect(
      (await dao.getById('terminal'))!.state,
      MissingMediaDao.statePendingRepair,
    );
    final due = await dao.dueForRequest(2000);
    expect(due.map((d) => d.mediaId).toSet(), {'pending', 'terminal'});
    expect(due.every((d) => d.forceRepair), isTrue);
  });
