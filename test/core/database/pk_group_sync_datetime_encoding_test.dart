...
void main() {
  group('PK group sync DateTime encoding', () {
    test(
      'PkGroupEntryDeferredSyncOpsDao.upsert round-trips DateTime without '
      'drifting into year ~58,000',
      () async {
    ...
    test(
      'PkGroupEntryDeferredSyncOpsDao.markRetried stamps lastRetryAt to '
      'current wall clock and increments retryCount',
      () async {
        final db = AppDatabase(NativeDatabase.memory());
        addTearDown(db.close);
    ...
    test(
      'PkGroupSyncAliasesDao.upsertAlias round-trips createdAt within '
      'the current year',
      () async {
        final db = AppDatabase(NativeDatabase.memory());
        addTearDown(db.close);
    ...
    test(
      '_deferPkBackedMemberGroupEntryOp writes seconds-encoded createdAt '
      'via the adapter applyFields path',
      () async {
        final db = AppDatabase(NativeDatabase.memory());
        addTearDown(db.close);
    ...
    test(
      '_recordPkGroupAliasIfNeeded writes seconds-encoded createdAt when '
      'applyFields materializes an aliased PK group row',
      () async {
        final db = AppDatabase(NativeDatabase.memory());
        addTearDown(db.close);
        ...
        expect(
          ...
          inInclusiveRange(before.year - 1, before.year + 1),
          reason:
              'Adapter-recorded alias createdAt should be seconds-encoded.',
        );
      },
    );
  });
