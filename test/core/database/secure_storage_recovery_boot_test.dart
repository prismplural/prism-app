...
void main() {
  ...
  group('integration: corrupted blob across all slots → unrecoverable', () {
    ...
    test(
      'prism.db exists, every secure-storage read throws cipher → '
      'unrecoverable + appDbKey=unreadable',
      () async {
  ...
  group('integration: sync-slot recovery → repair-pending → retry write-back',
      () {
    late Directory tempDir;
    late _SecureStorageStub storageStub;
    late KeychainDegradedStateService degradedStateService;
    ...
    test(
      ...
      () async {
        ...
        expect(
          await isKeychainRepairPending(),
          isTrue,
          reason: 'write-back failed → flag must stay set for next boot retry',
        );
  ...
  group('integration: sync DB cross-recovery from app primary', () {
    ...
    test(
      ...
      () async {
        ...
        storageStub.throwOnReadKeyQueue[
            '${kSyncDatabaseKeyStorageKey}_staging'] = [_cipherException()];
  ...
  group('integration: pairing state machine offline-safe', () {
    ...
    test(
      ...
      () async {
        ...
        expect(
          storageStub.store.containsKey('prism_sync.sync_id'),
          isFalse,
        );
        expect(
          storageStub.store.containsKey('prism_sync.device_id'),
          isFalse,
        );
