...
void _reshapeAgeToIntegerAndDowngrade(
  ...
  try {
    ...
    if ((ageType as String).toUpperCase() != 'INTEGER') {
      throw StateError(
        'Seed setup failed: age column is $ageType, not INTEGER',
      );
    }
...
void main() {
  group('schema v30 → v31: age column INT → TEXT', () {
    test('migrates integer age to string and preserves NULL age', () async {
      ...
      expect(
        await _ageColumnIsText(upgraded),
    ...
    test('row count and primary-key identity are preserved', () async {
      ...
      final dbFile = await _seedDb(
        ...
        ageById: {for (var i = 0; i < n; i++) 'member-$i': i.isEven ? i : null},
      );
    ...
    test('v29 public upgrade adds v30 columns before casting age', () async {
      // Real public path from 0.10.1 (v29) to 0.11.0 (v31).
      final createdAt = DateTime.utc(2026, 5, 31, 12);
      final dbFile = await _seedDb(
        'prism_migration_v29_to_v31_public_',
        seed: (db) async {
          ...
          await db
              ...
              .insert(
                MembersCompanion.insert(
                  ...
                  name: 'First User',
                  createdAt: createdAt,
                  displayName: const Value('First Display'),
                ),
              );
          await db
              .into(db.members)
          ...
          await db
              ...
              .insert(
                ConversationsCompanion.insert(
                  id: 'dm-1',
                  createdAt: createdAt,
                  lastActivityAt: createdAt.add(const Duration(minutes: 2)),
                  isDirectMessage: const Value(true),
                  participantIds: const Value(
                    '["member-first","member-other"]',
                  ),
                ),
              );
          await db
              ...
              .insert(
                ChatMessagesCompanion.insert(
                  ...
                  conversationId: 'dm-1',
                  authorId: const Value('member-first'),
                ),
              );
          await db
        ...
        ageById: {'member-first': 29, 'member-other': null},
        userVersion: 29,
        extraSetup: (rawDb) {
          rawDb.execute(
          ...
          rawDb.execute(
            'DROP INDEX IF EXISTS idx_media_attachments_member_deleted',
          );
          rawDb.execute('ALTER TABLE media_attachments DROP COLUMN member_id');
          rawDb.execute('ALTER TABLE media_attachments DROP COLUMN tag');
        },
      );
      ...
      final settingsColumns =
          (await upgraded
              ...
              .toSet();
      expect(settingsColumns, contains('members_show_groups'));

      final mediaColumns =
          (await upgraded
                  .customSelect('PRAGMA table_info(media_attachments)')
                  .get())
              .map((row) => row.read<String>('name'))
              .toSet();
      expect(mediaColumns, containsAll(['member_id', 'tag']));

      final settings = await upgraded.systemSettingsDao.getSettings();
      expect(settings.membersShowGroups, isTrue);

      final first = await upgraded.membersDao.getMemberById('member-first');
      expect(first, isNotNull);
      expect(first!.age, '29');
      expect(first.displayName, 'First Display');

      final other = await upgraded.membersDao.getMemberById('member-other');
      expect(other, isNotNull);
      expect(other!.age, isNull);

      final dm = await upgraded.conversationsDao.getConversationById('dm-1');
      expect(dm, isNotNull);
      expect(dm!.isDirectMessage, isTrue);
      expect(dm.participantIds, '["member-first","member-other"]');

      final msgCount = await upgraded
          .customSelect(
            "SELECT COUNT(*) AS c FROM chat_messages WHERE conversation_id = 'dm-1'",
          )
          .getSingle();
      expect(msgCount.read<int>('c'), 1);

      final attachment = await upgraded.mediaAttachmentsDao.getById(
        'attachment-1',
      );
      expect(attachment, isNotNull);
      expect(attachment!.messageId, 'msg-1');
      expect(attachment.memberId, '');
      expect(attachment.tag, '');
      ...
      final settingsRepo = DriftSystemSettingsRepository(
        upgraded.systemSettingsDao,
        null,
      );
      final memberRepo = DriftMemberRepository(upgraded.membersDao, null);
      final conversationRepo = DriftConversationRepository(
        upgraded.conversationsDao,
        null,
      );
      final messageRepo = DriftChatMessageRepository(
        upgraded.chatMessagesDao,
        null,
      );
      final mediaRepo = DriftMediaAttachmentRepository(
        upgraded.mediaAttachmentsDao,
        null,
      );

      final domainSettings = await settingsRepo.getSettings();
      expect(domainSettings.membersShowGroups, isTrue);

      final domainMembers = await memberRepo.getAllMembers();
      final domainFirst = domainMembers.firstWhere(
        (member) => member.id == 'member-first',
      );
      expect(domainFirst.age, '29');

      final domainConversations = await conversationRepo.getAllConversations();
      final domainDm = domainConversations.singleWhere(
        (conversation) => conversation.id == 'dm-1',
      );
      expect(domainDm.isDirectMessage, isTrue);
      expect(domainDm.participantIds, ['member-first', 'member-other']);

      final domainMessages = await messageRepo.getMessagesForConversation(
        'dm-1',
      );
      expect(domainMessages.single.authorId, 'member-first');

      final domainAttachments = await mediaRepo.getForMessage('msg-1');
      expect(domainAttachments.single.memberId, '');
      expect(domainAttachments.single.tag, '');
    });

    test(
      'v31 step composes with the full migration chain (v28 → v31)',
      () async {
        // Current schema reshaped back to v28, then migrated through v31.
        final dbFile = await _seedDb(
          ...
          extraSetup: (rawDb) {
            rawDb.execute(
              'ALTER TABLE members DROP COLUMN pk_avatar_cached_url',
            );
            rawDb.execute(
              'ALTER TABLE system_settings DROP COLUMN members_show_groups',
            );
        ...
        expect(
          row!.age,
          '19',
          reason: 'integer age cast to string across chain',
        );
