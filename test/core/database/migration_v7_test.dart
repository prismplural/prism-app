...
Future<void> _seedV6Db(
  ...
  try {
    ...
    // Flattened v18→v19: pending_pk_op on member_group_entries — drop so
    // onUpgrade can re-add it without "duplicate column" errors.
  ...
  group('schema v7 migration', () {
    ...
    test('fresh v7 schema has new columns, composite + orphan PK indexes, '
      ...
      expect(
        oldIndex,
        isEmpty,
        reason: 'old single-column index must not exist',
      );
    ...
    test(
      'v6 → v7 upgrade adds columns, indexes, and sets mode to notStarted',
      () async {
    ...
    test('v6 → v7 with duplicate (pluralkit_uuid, member_id) pairs: '
        "logs blockers, sets mode to 'blocked', skips composite index", () async {
      final tempDir = Directory.systemTemp.createTempSync(
        'prism_migration_v7_blocked_',
      );
      addTearDown(() {
        if (tempDir.existsSync()) tempDir.deleteSync(recursive: true);
      });
    ...
    test('v6 → v7 with duplicate (pluralkit_uuid, NULL member_id) orphan rows: '
        "logs blockers, sets mode to 'blocked'", () async {
      final tempDir = Directory.systemTemp.createTempSync(
        'prism_migration_v7_orphan_dup_',
      );
      addTearDown(() {
        if (tempDir.existsSync()) tempDir.deleteSync(recursive: true);
      });
    ...
    test('blocked-mode recovery installs composite + orphan indexes and drops '
        'the legacy single-column PK index', () async {
      final tempDir = Directory.systemTemp.createTempSync(
