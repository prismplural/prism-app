...
void main() {
  group('ensureFrontingMemberCheckConstraint orphan scrub', () {
    test(
      'purges (session_type=0, member_id=NULL) rows before installing CHECK',
      () async {
        ...
        // Non-deleted orphan with no pluralkit_uuid — also must be scrubbed.
        await db.customStatement(
          'INSERT INTO fronting_sessions '
          '(id, session_type, start_time, end_time, member_id, '
          ' co_fronter_ids, is_health_kit_import, is_deleted) '
        ...
        final survivors = await db
            .customSelect('SELECT id FROM fronting_sessions ORDER BY id')
            .get();
        expect(
          survivors.map((r) => r.read<String>('id')).toList(),
          ['healthy', 'sleep'],
        );
        ...
        expect(
          ...
          throwsA(anything),
          reason: 'CHECK (session_type != 0 OR member_id IS NOT NULL) '
  ...
  group('ensurePkFrontingIndexes orphan scrub', () {
    test(
      'purges orphans before installing the PK orphan unique index, '
      'so duplicate (uuid, NULL) rows do not block index creation',
      () async {
        final db = AppDatabase(NativeDatabase.memory());
        addTearDown(db.close);
        await db.customSelect('SELECT 1').get();
