...
/// Migration test for defaultSleepQuality — schema v56 → v57.
///
/// Adds `default_sleep_quality TEXT` (nullable) to `system_settings`
/// for the device-local default sleep quality preference.
void main() {
  group('migration v56 → v57 (defaultSleepQuality)', () {
    test('fresh database has default_sleep_quality column defaulting to null',
        () async {
      ...
      final colInfo = await db.customSelect(
        ...
        "WHERE name = 'default_sleep_quality'",
      ).get();

      expect(colInfo, hasLength(1));
      expect(colInfo.first.read<String>('name'), 'default_sleep_quality');
      // Nullable column with no DEFAULT — dflt_value is null.
      expect(colInfo.first.read<String?>('dflt_value'), isNull);
    });

    test('upgrade from v56 adds column and preserves existing row', () async {
      final tempDir =
          Directory.systemTemp.createTempSync('prism_migration_v57_');
      addTearDown(() {
      ...
      // Stub a v56 system_settings table without the new column.
      ...
      try {
        ...
        ''');
        rawDb.execute(
          'INSERT INTO system_settings (id, system_name, theme_corner_style) '
      ...
      final rows = await db.customSelect(
        'SELECT id, system_name, theme_corner_style, default_sleep_quality '
        'FROM system_settings',
      ).get();
      expect(rows, hasLength(1));
      expect(rows.single.read<String>('id'), 'singleton');
      expect(rows.single.read<String?>('system_name'), 'Test System');
      // Existing row: theme_corner_style preserved, new column defaults to null.
      expect(rows.single.read<int>('theme_corner_style'), 1);
      expect(rows.single.read<String?>('default_sleep_quality'), isNull);
    });
  });
}
