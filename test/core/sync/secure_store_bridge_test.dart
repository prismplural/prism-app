...
/// forwards byte buffers to `ffi.seedSecureStore`. Since `_seedRustStore`
/// takes an FFI handle that can't be constructed in a unit test, we test
/// the pure `buildSeedEntries` that `_seedRustStore` now delegates to.
...
void main() {
  group('buildSeedEntries', () {
    test('returns null for empty keychain (nothing to seed)', () {
      expect(buildSeedEntries(const {}), isNull);
    });

    test('returns null when the keychain only has unrelated entries', () {
      final result = buildSeedEntries({
    ...
    test('seed_then_drain_preserves_dynamic_epoch_keys_end_to_end', () {
      ...
      // then restarts and calls `_seedRustStore`. The byte map passed
      // to Rust must contain every dynamic key.
      ...
      expect(entries, isNotNull, reason: 'must emit a non-null byte map');
      final decoded = entries!;

      // Static allow-list keys survive.
      expect(
        decoded['wrapped_dek'],
        orderedEquals(base64Decode('d3JhcHBlZA==')),
      );
      expect(
        decoded['device_id'],
        orderedEquals(base64Decode('ZGV2aWNlLWFiYw==')),
      );
      expect(decoded['sync_id'], orderedEquals(base64Decode('c3luYy1hYmM=')));

      // Dynamic prefix scan picks up epoch + runtime keys.
      expect(
        decoded['epoch_key_1'],
        orderedEquals(base64Decode('ZXBvY2gxa2V5')),
        reason: 'seed must carry epoch_key_1 from readAll()',
      );
      expect(
        decoded['epoch_key_2'],
        orderedEquals(base64Decode('ZXBvY2gya2V5')),
        reason: 'seed must carry epoch_key_2 from readAll()',
      );
      expect(
        decoded['runtime_keys_foo'],
        orderedEquals(base64Decode('Zm9vcnVudGltZQ==')),
    ...
    test('out-of-range epoch keys (epoch > cached epoch) still propagate', () {
      ...
      final decoded = entries!;
      expect(decoded['epoch_key_17'], orderedEquals(base64Decode('a2V5MTc=')));
    });
    ...
    test('ignores foreign-prefixed entries (other_app.*)', () {
      ...
      expect(entries, isNotNull);
      final decoded = entries!;
      expect(decoded['epoch_key_1'], orderedEquals(base64Decode('b3Vycw==')));
