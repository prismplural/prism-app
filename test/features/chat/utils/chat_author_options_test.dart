...
Member _member(String id, {bool isActive = true}) => Member(
  id: id,
  name: id,
...
void main() {
  ...
  group('chatAuthorCandidateIds', () {
    ...
    test('DM with empty participantIds returns all active member ids + unknown',
        () {
      final ids = chatAuthorCandidateIds(_dm([]), [alice, bob]);
      expect(ids, containsAll(['alice', 'bob', unknownSentinelMemberId]));
    });

    test('named group returns all active member ids plus unknown', () {
      final ids = chatAuthorCandidateIds(
    ...
    test('includesAllMembers returns all active member ids plus unknown', () {
      final ids = chatAuthorCandidateIds(
        _group(includesAllMembers: true),
        [alice, bob, carol],
      );
      expect(
        ids,
    ...
    test('currentAuthorId is included in result even if not in activeMembers',
        () {
    ...
    group('legacy DM (isDirectMessage=false, untitled, 2 participants)', () {
      ...
      test('returns only participant members — same set as proper DM', () {
        final legacyIds = chatAuthorCandidateIds(
          _legacyDm(['alice', 'bob']),
          [alice, bob, carol],
        );
        final properDmIds = chatAuthorCandidateIds(
          _dm(['alice', 'bob']),
          [alice, bob, carol],
        );
        expect(legacyIds, equals(properDmIds),
            reason:
                'legacy DM should produce the same candidate set as a proper DM');
        expect(legacyIds, isNot(contains('carol')),
            reason: 'carol is not a participant — must be excluded');
        expect(legacyIds, isNot(contains(unknownSentinelMemberId)),
  ...
  group('chatAuthorCandidates', () {
    test('DM: returns participant members; unknown omitted unless explicit', () {
    ...
    test('named group returns all active members plus unknown at end', () {
      final result = chatAuthorCandidates(
        _group(participantIds: ['alice', 'bob']),
        [alice, bob, carol],
        l10n,
    ...
    test('includesAllMembers returns all active members plus unknown at end',
        () {
      final result = chatAuthorCandidates(
        _group(includesAllMembers: true),
        [alice, bob, carol],
    ...
    group('ordering — currentAuthorId', () {
      ...
      test('unknown sentinel as currentAuthorId is included (pinned last by default)',
          () {
      ...
      test(
          'departed current author (not in activeMembers) is at index 0 when '
        ...
        // the rest of the list should still have alice and bob
        expect(result.map((m) => m.id), containsAll(['alice', 'bob']));
      });

      test('currentAuthorId null leaves order unchanged; unknown last for non-DMs',
          () {
        // For a non-DM, Unknown is always appended last.
        final result = chatAuthorCandidates(
          _group(participantIds: ['alice', 'bob']),
          [alice, bob],
          l10n,
        );
        expect(result.last.id, unknownSentinelMemberId);
      });
    });
  });
}
