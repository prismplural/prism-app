...
void main() {
  ...
  //
  // When the PK push-disabled guard fires at the notifier layer, the importer
  // must propagate it down to SpMapper so every imported member is created
  // with `pluralkitSyncIgnored: true`. Without this, a large SP import would
  // flood the "local-only members" banner with every imported member.
  // ---------------------------------------------------------------------------

  group('markPushIgnored guard', () {
    test('default (false) leaves pluralkitSyncIgnored == false', () async {
      final repos = _makeFakeRepos();
      final importer = SpImporter(httpClient: _FakeHttpClient());
      ...
      final imported = await repos.memberRepo.getAllMembers();
      ...
      for (final member in imported) {
        expect(
          ...
          isFalse,
          reason:
              'Default markPushIgnored=false must preserve existing import '
              'behavior — members are eligible for push.',
        );
      }
    });

    test(
      'markPushIgnored=true sets pluralkitSyncIgnored on SP members AND '
      'custom-front-as-member rows',
      () async {
        final repos = _makeFakeRepos();
        final importer = SpImporter(httpClient: _FakeHttpClient());

        // Two regular SP members + one custom front (importAsMember by
        // default with no disposition map) → three total member rows.
        final data = SpExportData(
          members: const [
            SpMember(id: 'sp-a', name: 'Alice'),
            SpMember(id: 'sp-b', name: 'Bob'),
        ...
        expect(
          ...
          hasLength(3),
          reason:
              'Expected two SP members + one custom-front-as-member row to '
              'all be imported.',
        );
        for (final member in imported) {
          expect(
            ...
            isTrue,
            reason:
                'markPushIgnored=true must mark every imported member '
                '(${member.name}) as pluralkitSyncIgnored so the local-only '
                'members banner does not flood after a large SP import.',
          );
        }
      },
    );
