...
void main() {
  group('PRISM1 rescue importer (legacy-shape branch)', () {
    ...
    test('PK legacy-shape rows fan out per pk_member_ids_json with '
        'deterministic v5 ids and lossy boundaries preserved', () async {
      ...
      // difference.
      expect(byId[alexId]!.startTime.toUtc(), DateTime.parse(start).toUtc());
      expect(byId[alexId]!.endTime!.toUtc(), DateTime.parse(end).toUtc());
    });

    test('rescue-then-API-reimport corrects PK boundaries via fresh HLCs '
        '(spec §4.7 critical test)', () async {
      ...
      // rescue collision upsert" group) — which catches the Regression:         // #6 regression where the previous create-then-catch pattern
      ...
      await importService.frontingSessionRepository.updateSession(
        domainSession!.copyWith(startTime: correctStart, endTime: correctEnd),
      );
    ...
    test('PK short id with no matching local member is counted as skipped, '
        'not crashed', () async {
    ...
    test('native legacy multi-member fan-out: primary keeps id, '
        'co-fronters get migrationFrontingNamespace v5 ids', () async {
      ...
      const uuid = Uuid();
      final co1Id = uuid.v5(migrationFrontingNamespace, '$sessionId:$coId1');
      final co2Id = uuid.v5(migrationFrontingNamespace, '$sessionId:$coId2');
      final rows = await db.frontingSessionsDao.getAllSessions();
      final byId = {for (final r in rows) r.id: r};
    ...
    test('legacy detection: empty-but-present coFronterIds — orphan native '
        'row routes through rescue and assigns Unknown sentinel', () async {
    ...
    test('legacy detection: completely absent coFronterIds (real old shape) '
      ...
      expect(
        ...
        isTrue,
        reason:
            'broadened detection must route this to rescue and '
    ...
    test('legacy detection: completely absent legacy keys on a solo PK row '
        '(real old shape) routes through rescue and derives canonical '
        '(switch, member) v5 id', () async {
      ...
      expect(
        ...
        derivedId,
        reason:
            'broadened detection must route this through rescue, '
    ...
    test('corrupt co_fronter_ids JSON falls back to single-member migration '
        'and logs the row id (spec §6 edge case)', () async {
    ...
    test('new-shape rows route through the standard import path, no fan-out, '
        'no v5 derivation, sessionType honored', () async {
      ...
      final rows = await db.frontingSessionsDao.getAllSessions();
      final byId = {for (final r in rows) r.id: r};
      expect(byId['new-shape-row']!.sessionType, SessionType.normal.index);
      expect(byId['new-shape-sleep']!.sessionType, SessionType.sleep.index);
    ...
    test(
      ...
      'same row) routes through the rescue path (Regression: #2 regression)',
      () async {
    ...
    test('PK rescue with empty pkMemberIdsJson but headmateId present derives '
        'deterministic id from local member.pluralkit_uuid, '
        'driven by real DataExportService.buildExport', () async {
      // Real-export-driven test. The previous
      // version hand-built `coFronterIds: []` JSON — a key the real
      // exporter would never emit (`toJson` skips empty lists). With
      // the envelope-level rescueLegacyFields marker, the row no longer
      // needs any per-row legacy key to
      ...
      await sourceFronting.createSession(
        FrontingSession(
          id: legacySessionId,
      ...
      final exportModel = await exportSvc.buildExport(
      ...
      expect(
        exportModel.rescueLegacyFields,
        isTrue,
        reason:
            'buildExport(includeLegacyFields: true) must set '
            'rescueLegacyFields on the envelope',
      );
      final exportedSessionJson = exportModel.frontSessions.single.toJson();
      expect(
        exportedSessionJson.containsKey('coFronterIds'),
        isFalse,
        reason:
            'real exporter omits empty coFronterIds — this is the '
            'shape that motivated the envelope marker',
      );
      expect(
        exportedSessionJson.containsKey('pkMemberIdsJson'),
        isFalse,
        reason:
            'real exporter omits null pkMemberIdsJson — same '
            'as above',
      );
      ...
      try {
        ...
        final freshMemberRepo = DriftMemberRepository(freshDb.membersDao, null);
        ...
        await freshMemberRepo.createMember(
          Member(
            id: memberLocalId,
            name: 'Fallback',
            emoji: 'F',
        ...
        expect(after.single.startTime.toUtc(), DateTime.utc(2026, 4, 1, 9, 30));
      } finally {
        await freshDb.close();
      }
    });

    test('orphan native row (member_id NULL) round-trips through '
        'buildExport(includeLegacyFields: true) and lands on the Unknown '
        'sentinel via the envelope-marker rescue path', () async {
      ...
      final orphanJson = exportModel.frontSessions
          ...
          .toJson();
      expect(
        orphanJson.containsKey('headmateId'),
        isFalse,
        reason: 'null member_id is not serialized',
      );
      expect(
        orphanJson.containsKey('coFronterIds'),
        isFalse,
      ...
      try {
        ...
        expect(
          ...
          isTrue,
          reason:
              'orphan row must be rerouted to the Unknown sentinel '
        ...
        expect(
          ...
          sentinelId,
          reason:
              'orphan must land on the Unknown sentinel, not be '
    ...
    test(
      ...
      () async {
        ...
        // non-deterministic id would re-introduce the nondeterministic-id bug —
      ...
      () async {
        ...
        final json = _envelope(
          frontSessions: [
            {
              'id': 'sess-a',
              'startTime': DateTime.utc(2026, 4, 1, 6, 42).toIso8601String(),
            ...
            {
              'id': 'sess-b',
              'startTime': DateTime.utc(2026, 4, 1, 8, 50).toIso8601String(),
    ...
    test('three-row cascade: legacy A→B→C all touching for the same headmate '
        'collapse to one row through the rescue path', () async {
      const host = 'host';
      await memberRepo.createMember(
        Member(
          id: host,
      ...
      final json = _envelope(
        frontSessions: [
          ...
          },
          {
            ...
            'endTime': DateTime.utc(2026, 4, 1, 9, 30).toIso8601String(),
            'headmateId': host,
            'coFronterIds': <String>[],
          },
        ],
      ...
      () async {
        ...
        final json = _envelope(
          frontSessions: [
            ...
            {
              'id': 'b',
              'startTime': DateTime.utc(2026, 4, 1, 8, 55).toIso8601String(),
              'endTime': DateTime.utc(2026, 4, 1, 9, 7).toIso8601String(),
              'headmateId': host,
              'coFronterIds': <String>[],
            },
    ...
    test('notes concatenate when both legacy rows carry notes', () async {
      const host = 'host';
      await memberRepo.createMember(
        Member(
          id: host,
    ...
    test('open-ended merge: B has null endTime in the legacy file → merged '
        'row stays open-ended after rescue', () async {
      const host = 'host';
      await memberRepo.createMember(
        Member(
          id: host,
    ...
    test('multi-member sanity: rescue native multi-member fans out then '
        'merges only same-member adjacency', () async {
      const zari = 'z';
      const aimee = 'a';
      for (final m in [zari, aimee]) {
        await memberRepo.createMember(
  ...
  group('rescue importer sync semantics + structured dedup (PR G)', () {
    ...
    test('rescue body runs under SyncRecordMixin.suppress; '
        'final-state ops emit per surviving id (review finding #9)', () async {
    ...
    test('soft-deleted member PK uuid still resolves through includeDeleted '
        'DAO during rescue (review finding #10)', () async {
      ...
      expect(activeOnly.where((m) => m.id == memberId).toList(), isEmpty);
    ...
    test(
      ...
      () async {
        ...
        // is preserved.
        expect(rows, hasLength(2));
        expect(rows.map((r) => r.notes).toSet(), {'Note A', 'Note B'});
    ...
    test('PkSessionMemberKey: null memberId differs from empty string and '
  ...
  // -- Regression: rescue → API corrective end-to-end ----------
  //
  ...
  group('rescue → API corrective re-import end-to-end', () {
    final storageStub = _SecureStorageStub();

    setUp(storageStub.setup);
    tearDown(storageStub.teardown);

    test('A → A+B → A: rescue fans out to stale rows, API corrective '
        'tombstones stale ones and leaves A currently fronting', () async {
      const alexLocalId = 'alex-local';
      const ezraLocalId = 'ezra-local';
      const alexPkUuid = '11111111-1111-4111-8111-111111111111';
      const ezraPkUuid = '22222222-2222-4222-8222-222222222222';
      ...
      expect(
        afterRescue.map((s) => s.id).toSet(),
        rescueIds,
        reason: 'rescue importer must fan out 4 rows for A→A+B→A',
      );
      expect(
        afterRescue.every((s) => s.endTime != null),
        isTrue,
      ...
      expect(
        after.map((s) => s.id).toSet(),
        {canonicalAlex, canonicalEzra},
        reason:
            'only canonical entrant rows survive the corrective '
            're-import; stale rescue fan-outs are tombstoned',
      );
      expect(
        after.any((s) => s.id == staleAlexAtSw2),
        isFalse,
        reason: 'continuous-A stale row at sw-2 must be tombstoned',
      );
      expect(
        after.any((s) => s.id == staleAlexAtSw3),
        isFalse,
      ...
      expect(
        ...
        isNull,
        reason:
            'A is currently fronting per API; corrective re-import '
