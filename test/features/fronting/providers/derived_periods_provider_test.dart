...
void main() {
  group('derivedPeriodsProvider', () {
    ...
    test('reads from same upstream snapshot return identical period list '
        '(memoization within a single emission)', () async {
      final container = ProviderContainer(
        overrides: [
          unifiedHistoryOverlapProvider.overrideWith(
            (ref) => Stream.value(
        ...
        overrides: [
          ...
          allMembersProvider.overrideWith(
            (ref) => Stream.value([
              ...
              Member(id: 'v', name: 'V', createdAt: DateTime(2026, 1, 1)),
            ]),
          ),
        ],
      );
    ...
    test('production overlap-query path catches a long-running host '
        '(real Drift, no provider override)', () async {
      // Regression: instead of overriding the upstream stream
      ...
      // lookback window.
      final allActive = <String>{for (final p in periods) ...p.activeMembers};
      expect(
        allActive,
        contains('host'),
        reason:
    ...
    test('provider range clamps a 400-day host so the sweep stays inside the '
      ...
      final providerRangeStart = referenceNow.subtract(
      ...
      for (final p in periods) {
        expect(
          p.start.isBefore(providerRangeStart),
          isFalse,
      ...
      final hostPeriods = periods.where(
        (p) => p.activeMembers.contains('host'),
      );
      expect(
        hostPeriods,
        isNotEmpty,
    ...
    test('mutation after subscription: a row written with start ≈ now '
        'appears in derived periods without explicit invalidation', () async {
    ...
    test(
      ...
      () async {
        // Regression: (rewritten post-rangeEnd-removal): the
        ...
        expect(
          ...
          lessThan(1),
          reason:
              'open period end must be ~now, not 30 days out '
              '(was: ${open.end})',
        );
      },
    );
    ...
    test(
      ...
      () async {
        // Regression: even though the DAO's +30d lookahead
    ...
    test(
      ...
      () async {
        ...
        final controller =
            StreamController<DerivedPeriodsInputBundle>.broadcast();
        addTearDown(controller.close);
    ...
    test(
      ...
      () async {
        // Regression: with the bundle still carrying
        ...
        expect(
          ended.single.start.difference(startTime).abs().inMilliseconds,
          lessThan(1500),
        );
        expect(
