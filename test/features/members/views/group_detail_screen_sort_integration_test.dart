...
Future<void> _seedGroup(
  ...
  await db.into(db.memberGroups).insert(
        MemberGroupsCompanion.insert(
          ...
          sortState:
              Value(MemberGroupMapper.encodeSortStateForColumn(sortState)),
        ),
      );
}
    ...
    overrides: [
      ...
      groupEntriesProvider.overrideWith(
        (ref, groupId) => Stream.value(
          entries.where((e) => e.groupId == groupId).toList(),
        ),
      ),
...
void main() {
  ...
  testWidgets(
    'scenario 1: opens group with 3 members; pick Name A-Z; '
    'chip + sort + repo call',
    (tester) async {
  ...
  testWidgets(
    'scenario 2: drag in manual mode persists snapshot via repo',
    (tester) async {
  ...
  test(
    'scenario 3: repo persistence: setGroupManualOrderSnapshot writes '
    'survive a fresh read from the same DB',
    () async {
      ...
      final result =
          await repo.setGroupManualOrderSnapshot('g', ['e2', 'e1']);
  ...
  test(
    'scenario 4: cross-device sync convergence — last applyFields wins '
    'the complete (mode, order) pair',
    () async {
      final db = AppDatabase(NativeDatabase.memory());
      addTearDown(db.close);
      ...
      final state = await _readSortState(db, 'g');
      expect(state.manualOrder, ['e1', 'e2'],
          reason: 'B applied last → B wins atomically');
    },
  );

  test(
    'scenario 5: cross-device sort-mode race — single-field LWW means '
    'one device wins the whole (mode, order) pair, never a split',
    () async {
      final db = AppDatabase(NativeDatabase.memory());
      addTearDown(db.close);
      final entity = _entityFor(db, 'member_groups');
  ...
  test(
    'scenario 6: cross-record add + reorder race — '
    'unindexed live entry appears at end',
    () async {
      final db = AppDatabase(NativeDatabase.memory());
      addTearDown(db.close);
      ...
      final container = ProviderContainer(
        overrides: [
          ...
          groupByIdProvider.overrideWith(
            (ref, _) => Stream.value(_groupModel(
              id: 'g',
              name: 'g',
              sortState: state,
            )),
  ...
  test(
    'scenario 7: cross-record remove + reorder race — '
    'tombstoned id in manualOrder is filtered out at read',
    () async {
      ...
      final container = ProviderContainer(
        overrides: [
          ...
          groupByIdProvider.overrideWith(
            (ref, _) => Stream.value(
              _groupModel(id: 'g', name: 'g', sortState: state),
            ),
          ),
          groupEntriesProvider.overrideWith(
            (ref, _) => Stream.value(const [
  ...
  // throws.
  test(
    'scenario 8: app-kill mid-write leaves local DB consistent; '
    'emitGroupSyncState re-broadcasts the new state',
    () async {
      final db = AppDatabase(NativeDatabase.memory());
      addTearDown(db.close);
      final dao = _TestMemberGroupsDao(db);
      ...
      expect(groupUpdate.isNotEmpty, isTrue,
  ...
  test(
    'scenario 9: corrupt remote sort_state is rejected; '
    'next local _groupFields emits previous valid state',
    () async {
      final db = AppDatabase(NativeDatabase.memory());
      addTearDown(db.close);
      final entity = _entityFor(db, 'member_groups');
      ...
      final fields = groupUpdate['fields'] as Map<String, dynamic>;
      final emitted = tryDecodeSortState(fields['sort_state'] as String?);
      expect(emitted, preGarbage,
          reason: 'garbage cannot round-trip through the local DB');
    },
  );

  // ── Scenario 10: whole-entity LWW invariant ─────────────────────────────
  // Device A renames group, device B reorders concurrently. Both emit
  // FULL _groupFields. Apply at a third peer. The newer-HLC device's
  // complete snapshot wins, including name AND sort_state.
  test(
    'scenario 10: whole-entity LWW — newer applyFields wins ALL fields '
    'in one shot (name + sort_state from the same device)',
    () async {
      final db = AppDatabase(NativeDatabase.memory());
      addTearDown(db.close);
      final entity = _entityFor(db, 'member_groups');

      // Initial: name = "Original", manualOrder = [e1, e2].
      await _seedGroup(
        db,
        id: 'g',
        name: 'Original',
      ...
      // Device A renames to "Renamed" but ships its prior sort_state
      // (still [e1, e2]).
      final deviceAFields = {
        'name': 'Renamed',
        'sort_state': MemberGroupMapper.encodeSortStateForColumn(
      ...
      // Device B reorders to [e2, e1] but ships its prior name "Original".
      final deviceBFields = {
        'name': 'Original',
        'sort_state': MemberGroupMapper.encodeSortStateForColumn(
          const GroupSortState(
      ...
      // Apply A then B. B applied last → B's complete snapshot wins.
      await entity.applyFields('g', deviceAFields);
      await entity.applyFields('g', deviceBFields);

      final row = await (db.select(db.memberGroups)
            ..where((g) => g.id.equals('g')))
          .getSingle();
      expect(row.name, 'Original',
          reason: 'B applied last; name comes from B');
      final state = tryDecodeSortState(row.sortState)!;
      expect(state.manualOrder, ['e2', 'e1'],
          reason: 'B applied last; sort_state comes from B (whole-entity LWW)');
    },
  );
