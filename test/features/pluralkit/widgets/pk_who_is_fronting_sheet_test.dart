...
void main() {
  ...
  test('(d) collapse rule: Prism [A,B] vs PK [A] → usePrism and cofront same set → 2 options', () {
    final options = computeOptions(
      localIds: {'A', 'B'},
      pkIds: {'A'},
      direction: PkSyncDirection.bidirectional,
  ...
  test('(f) PK empty, Prism non-empty → 2 options, keepMembers recommended', () {
    final options = computeOptions(
      localIds: {'A'},
      pkIds: {},
      direction: PkSyncDirection.bidirectional,
  ...
  testWidgets('(j) renders title, subtitle, and PkFronterChoiceCards', (tester) async {
    Set<String>? result;
  ...
  testWidgets('(k) tapping a card calls onResult with the resolved local IDs', (tester) async {
    Set<String>? result;
  ...
  testWidgets('(m) Prism empty + PK [Bob] → cards titled "Set Bob fronting" and "Leave no one fronting"', (tester) async {
    await tester.pumpWidget(
      _wrap(
        PkWhoIsFrontingSheet(
          localFronters: const [],
  ...
  testWidgets('(n) PK empty + Prism [Alice] → cards titled "Keep Alice fronting" and "Match PluralKit (no one fronting)"', (tester) async {
    await tester.pumpWidget(
      _wrap(
        PkWhoIsFrontingSheet(
          localFronters: [_e('A', 'Alice')],
