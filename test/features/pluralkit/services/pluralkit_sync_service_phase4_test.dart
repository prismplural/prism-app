...
class _FakeClient implements PluralKitClient {
  ...
  @override
  Future<PKSystem> getSystem() async => const PKSystem(id: 'sys', name: 'Test');
...
void main() {
  ...
  group('_importSwitch drop-bug fix', () {
    ...
    test(
      'switch with zero mapped members still persists with memberId=null',
      () async {
        final db = _makeDb();
        addTearDown(db.close);
  ...
  group('pushPendingSwitches — scoped push', () {
    test(
      'pushes only sessions started after linkedAt with mapped members',
      () async {
        final db = _makeDb();
        addTearDown(db.close);
    ...
    test('linkedAt == session.startTime boundary — session still pushed '
    ...
    test('rolls back first push when switch-out fails, leaves pluralkitUuid '
      ...
      expect(
        methods.contains('deleteSwitch'),
        isTrue,
        reason: 'rollback should call deleteSwitch on the start uuid',
      );
      final del = client.calls.firstWhere((c) => c.method == 'deleteSwitch');
  ...
  group('stale link 404 handling', () {
    ...
    test('pushPendingSwitches invokes onStaleLink callback on 404 '
        '(regression S3)', () async {
      final db = _makeDb();
      addTearDown(db.close);
  ...
  group('chain-gap repair — ongoing sessions closed on next import', () {
    ...
    test(
      'closes previously-ongoing session when a newer switch arrives',
      () async {
        final db = _makeDb();
        addTearDown(db.close);
        ...
        final b = sessionRepo.sessions.firstWhere(
          (s) => s.pluralkitUuid == 'sw-b',
        );
        expect(
          a.endTime,
          t2,
        ...
        expect(b.endTime, isNull, reason: 'newest session stays ongoing');
      },
    );
