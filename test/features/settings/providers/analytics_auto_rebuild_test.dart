...
void main() {
  group('frontingAnalyticsProvider auto-rebuild', () {
    test('a write to fronting_sessions rebuilds analytics without explicit '
        'invalidation (ticker contract)', () async {
      ...
      expect(
        ...
        1,
        reason:
            'analytics must auto-rebuild on a fronting_sessions '
            'write — no explicit invalidation should be required',
      );
      expect(after.totalTrackedTime, const Duration(hours: 1));
    });

    test('customStatement DELETE + db.notifyUpdates rebuilds analytics — '
        'truncate-path contract', () async {
