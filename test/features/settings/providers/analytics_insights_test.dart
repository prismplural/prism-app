...
void main() {
  group('computeInsights', () {
    ...
    test('gap > 25% fires gapAlert', () {
      ...
      expect(
        insights.any((i) => i.type == AnalyticsInsightType.gapAlert),
        isTrue,
      );
    });
    ...
    test(
      'member only in current (not in previous) does not fire quietMember',
      () {
        final current = makeAnalytics(
          memberStats: [makeMember('a'), makeMember('b')],
        );
    ...
    test(
      ...
      () {
        final current = makeAnalytics(
          memberStats: [makeMember('a', avgMinutes: 20)],
        );
        final previous = makeAnalytics(
    ...
    test('session drift < 25% does not fire', () {
      final current = makeAnalytics(
        memberStats: [makeMember('a', avgMinutes: 12)],
      );
      final previous = makeAnalytics(
        memberStats: [makeMember('a', avgMinutes: 10, sessionCount: 3)],
      );
    ...
    test('co-fronting pair fires coFrontingHighlight', () {
      ...
      final insights = computeInsights(current, null);
      expect(
        insights.any((i) => i.type == AnalyticsInsightType.coFrontingHighlight),
        isTrue,
      );
    });
    ...
    test(
      'previous null suppresses quietMember, sessionDrift, timeOfDayShift',
      () {
        final current = makeAnalytics(memberStats: [makeMember('a')]);
        final insights = computeInsights(current, null);
        expect(
    ...
    test('max 3 insights returned even when 5 conditions trigger', () {
      ...
      final current = makeAnalytics(
        ...
        rangeSpan: const Duration(days: 30),
        memberStats: [
          makeMember('a', avgMinutes: 30, timeOfDay: {'evening': 90}),
        ],
    ...
    test('fronting terminology overrides fronting-specific headlines', () {
      const pair = CoFrontingPair(
        memberIdA: 'a',
        memberIdB: 'b',
        totalTime: Duration(hours: 5),
      ...
      final coFrontInsights = computeInsights(
        ...
        null,
        names: const {'a': 'Alice', 'b': 'Bob'},
        sessionPluralLower: 'out sessions',
        togetherPastLabelLower: 'out together',
        activeTimeLabelLower: 'out time',
      );
      final coFront = coFrontInsights.firstWhere(
        (i) => i.type == AnalyticsInsightType.coFrontingHighlight,
      );
      expect(coFront.headline, 'Alice & Bob out together a lot this period');
      ...
      final quiet = quietInsights.firstWhere(
        (i) => i.type == AnalyticsInsightType.quietMember,
      );
      expect(quiet.headline, 'Bob has no out sessions this period');

      final shiftInsights = computeInsights(
        makeAnalytics(
          memberStats: [
            makeMember('a', timeOfDay: {'evening': 60}),
          ],
        ),
        makeAnalytics(
          memberStats: [
            makeMember('a', timeOfDay: {'morning': 60}),
          ],
        ),
        names: const {'a': 'Alice'},
        activeTimeLabelLower: 'out time',
      );
      final shift = shiftInsights.firstWhere(
        (i) => i.type == AnalyticsInsightType.timeOfDayShift,
      );
      expect(shift.headline, "Alice's out time of day shifted");
    });
    ...
    test('sessionDrift headline names the member when name provided', () {
      final current = makeAnalytics(
        memberStats: [makeMember('a', avgMinutes: 30)],
      );
      final previous = makeAnalytics(
        memberStats: [makeMember('a', avgMinutes: 10, sessionCount: 3)],
      );
