...
void main() {
  group('showQuickFrontProvider', () {
    ...
    test('returns true when systemSettingsProvider is loading', () {
      ...
      expect(container.read(showQuickFrontProvider), isTrue);
    });
    ...
    test('returns false when showQuickFront is false in settings', () {
      final container = makeContainer(
        settings: const SystemSettings(showQuickFront: false),
      );
      addTearDown(container.dispose);

      expect(container.read(showQuickFrontProvider), isFalse);
    });

    test('returns true when showQuickFront is explicitly true in settings', () {
      final container = makeContainer(
        settings: const SystemSettings(showQuickFront: true),
      );
      addTearDown(container.dispose);
