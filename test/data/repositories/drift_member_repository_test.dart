...
void main() {
  ...
  group('ensureUnknownSentinelMember', () {
    ...
    test(
      'second call is idempotent and reports wasCreated=false',
      () async {
    ...
    test(
      'two concurrent calls converge — exactly one wasCreated=true, '
      'no thrown exceptions, single row in the table',
      () async {
        ...
        final createdCount =
            results.where((r) => r.wasCreated).length;
        expect(
          createdCount,
          lessThanOrEqualTo(1),
    ...
    test(
      'simulated PK constraint violation in insertMember is caught and '
      'the winner is refetched',
      () async {
    ...
    test(
      'a non-constraint exception in insertMember is rethrown',
      () async {
