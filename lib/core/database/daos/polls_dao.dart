...
class PollsDao extends DatabaseAccessor<AppDatabase> with _$PollsDaoMixin {
    ...
    return (select(polls)
          ..where(
            (p) =>
                ...
                (p.expiresAt.isNull() | p.expiresAt.isBiggerOrEqualValue(now)),
          )
          ..orderBy([(p) => OrderingTerm.desc(p.createdAt)]))
        .get();
  }
  ...
  Future<List<Poll>> getClosedPolls() =>
      (select(polls)
            ..where((p) => p.isClosed.equals(true) & p.isDeleted.equals(false))
            ..orderBy([(p) => OrderingTerm.desc(p.createdAt)]))
          .get();
  ...
  /// Batch-insert polls in a single Drift `batch()` round-trip.
  /// Phase 6 SP importer; see `docs/plans/sp-import-perf-quick-wins.md`.
  Future<void> batchInsertPolls(List<PollsCompanion> rows) async {
    if (rows.isEmpty) return;
    await batch((b) => b.insertAll(polls, rows));
  }
  ...
  Future<void> updatePoll(PollsCompanion poll) {
    ...
    return (update(
      polls,
    )..where((p) => p.id.equals(poll.id.value))).write(poll);
  }
