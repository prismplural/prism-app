...
class FrontingActivityChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final daysWithData = dailyActivity
    ...
    final avgMinutes =
        dailyActivity.map((d) => d.totalMinutes).reduce((a, b) => a + b) /
        dailyActivity.length;
    final avgHours = (avgMinutes / 60).toStringAsFixed(1);
    final peakDate = peakIdx >= 0
        ? dateFormat.format(dailyActivity[peakIdx].date)
        : '';
    final firstDate = dateFormat.format(dailyActivity.first.date);
    final lastDate = dateFormat.format(dailyActivity.last.date);

    return Semantics(
      label: l10n.frontingActivityChartSemantics(peakHours, peakDate, avgHours),
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                Text(
                  l10n.frontingActivityChartTitle,
                  style: theme.textTheme.titleSmall?.copyWith(
                    fontWeight: FontWeight.bold,
                  ),
                ),
                Text(
                  l10n.frontingActivityChartAverageLabel(avgHours),
